 ...
 *
 * Mirrors packages/cli/src/analyze.ts - same analysis logic, same formulas
 * from the canonical metrics-impact-formula leaf package.
 */
import {
  type SkeletonData,
  Skeleton,
...
export function analyzeSkeletonData(skeletonData: SkeletonData): AnalysisReport {
  ...
  for (const animation of skeletonData.animations) {
    ...
    for (let i = 0; i < sampleCount; i++) {
      ...
      // canonical per-pose counting - the SAME walker every other path uses
      // (ADR 0002). The old local walk here dropped region/sequence vertices.
