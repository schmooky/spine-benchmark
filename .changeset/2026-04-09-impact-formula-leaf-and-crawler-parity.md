...
The crawler also unifies its `ImpactLevel` literal on `'veryHigh'` (camelCase, matching the i18n locale keys and the offline scorer). The previous `'very-high'` (kebab) literal is removed. **This is a breaking change for any consumer that compared the level by string;** that is why the crawler bumps to `0.2.0`.
