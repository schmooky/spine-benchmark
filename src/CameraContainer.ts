...
export class CameraContainer extends Container {
  ...
  lookAtChild(object: Spine) {
    ...
    document.getElementById("leftPanel")!.addEventListener("wheel", (event) => {
  ...
  public setMeshVisibility(isVisible: boolean) {
    ...
    const checkmark = this.contextMenu.querySelector(
      "div > div > div"
    ) as HTMLDivElement;
    if (checkmark) {
  ...
  public createDebugCanvas(containerId: string, name: string): void {
    ...
    if (!WebGLContextManager.canCreateContext()) {
      console.warn(
        "Maximum WebGL context limit reached. Cannot create more debug canvases."
      );
      return;
    }
    ...
    if (this.target!.skeleton.data.findSlotIndex(name) >= 0) {
      targetMeta = {
        type: "slot",
        index: this.target!.skeleton.data.findSlotIndex(name),
      };
    }
    ...
    if (targetMeta && targetMeta.type === "bone") {
      ...
      const debugRenderer = new DebugCanvasRenderer(
        this.app,
        renderTarget,
        containerId
      );
