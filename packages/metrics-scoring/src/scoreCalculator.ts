...
export function calculateMeshScore(metrics: any): number {
  ...
  const meshCount = metrics?.totalMeshCount ?? metrics?.activeMeshCount ?? 0;
  const totalVertices = metrics?.totalVertices ?? 0;
  const deformedMeshCount = metrics?.deformedMeshCount ?? 0;
  const weightedMeshCount = metrics?.weightedMeshCount ?? 0;
  ...
  if (meshCount > 0) {
    score -= Math.log2(meshCount / IDEAL_MESH_COUNT + 1) * 15;
  }
